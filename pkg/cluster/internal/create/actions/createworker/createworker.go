	...
	if !a.avoidCreation {
		...
		if len(addons) > 0 {
			...
			if awsEKSEnabled {
				...
				controlplaneHost, err := commons.ExecuteCommand(n, c, 3, 5)
				if err != nil {
					return errors.Wrap(err, "failed to get controlplane host")
				}
...
func isEmptyCredsMap(creds map[string]string, infra string) bool {
