...
Si nos referimos al control-plane, y como es evidente, en EKS y AKS no se podrá indicar una imagen, pero en Azure no-gestionado y en GCP si, siendo obligatorio para este último caso.
...
El _metadata_ de KeosCluster está dispuesto por los siguientes campos:
[cols="1,4,2,1"]
|===
...
|namespace
|namespace donde se despliegarán todos los recursos del _cluster_.  Por defecto, _cluster$$_$$[metadata.name]_.
|_cluster$$_$$my-cluster_
|Si
...
El _status_ de KeosCluster está dispuesto por los siguientes campos:
