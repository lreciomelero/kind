...
Como prerrequisito de instalación, se debe indicar el repositorio helm del que se descargará el chart de helm del _Cluster Operator_. Este campo permite indicar la url del repositorio, su tipo y si se trata de un repositorio autenticado.

[cols="1,4,2,1"]
|===
^|Nombre ^|Descripción ^|Ejemplo ^|Default

|_auth++_++required_ (*__boolean__*)
...
*Repositorios basados en Index*: http://[IP]:8080
|
...
NOTE: Los repositorios ecr nunca son autenticados. La autenticación se realizará mediante las credenciales contenidas en el secrets.yml, a través del sdk de golang. Este tipo de repositorio únicamente está soportado para instalaciones sobre AWS y EKS.
